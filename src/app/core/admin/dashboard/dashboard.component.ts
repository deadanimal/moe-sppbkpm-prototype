...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  getChart1() {
    ...
    lineSeries.events.on("datavalidated", function () {
      lineSeries.dataItems.each(function (dataItem) {
        ...
        if (qwert.count / 2 == Math.round(qwert.count / 2)) {
    ...
    columnSeries.columns.template.adapter.add("fill", function (fill, target) {
      let asdfg = target.dataItem.dataContext as any;
      let name = asdfg.realName;
      if (!colors[name]) {
  ...
  getChart3() {
    ...
    categoryAxis.dataItems.template.text = "{realName}";
    categoryAxis.adapter.add("tooltipText", function (tooltipText, target) {
      let err1 = categoryAxis.tooltipDataItem.dataContext as any;
      return err1.realName;
    });
    ...
    lineSeries.events.on("datavalidated", function () {
      lineSeries.dataItems.each(function (dataItem) {
        let err2 = dataItem.dataContext as any;
        // if count divides by two, location is 0 (on the grid)
        if (err2.count / 2 == Math.round(err2.count / 2)) {
          dataItem.setLocation("categoryX", 0);
        }
        // otherwise location is 0.5 (middle)
        else {
    ...
    // fill adapter, here we save color value to colors object so that each time the item has the same name, the same color is used
    columnSeries.columns.template.adapter.add("fill", function (fill, target) {
      let er3 = target.dataItem.dataContext as any;
      let name = er3.realName;
      if (!colors[name]) {
        colors[name] = chart.colors.next();
      }
      target.stroke = colors[name];
